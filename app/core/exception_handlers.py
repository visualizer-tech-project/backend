...
async def not_found_exception_handler(
    request: Request, exc: exceptions.NotFoundError
) -> JSONResponse:
    logger.warning(f'NotFoundError: {exc.detail}')
    return JSONResponse(
        status_code=404, content=NotFoundErrorSchema(detail=exc.detail).model_dump()
    )


async def forbidden_exception_handler(
    request: Request, exc: exceptions.ForbiddenError
) -> JSONResponse:
    logger.warning(f'ForbiddenError: {exc.detail}')
    return JSONResponse(
        status_code=403, content=ForbiddenErrorSchema(detail=exc.detail).model_dump()
    )


async def unauthorized_exception_handler(
    request: Request, exc: exceptions.UnauthorizedError
) -> JSONResponse:
    logger.warning(f'UnauthorizedError: {exc.detail}')
    return JSONResponse(
        status_code=401, content=UnauthorizedErrorSchema(detail=exc.detail).model_dump()
    )


async def bad_request_exception_handler(
    request: Request, exc: exceptions.BadRequestError
) -> JSONResponse:
    logger.warning(f'BadRequestError: {exc.detail}')
    return JSONResponse(
        status_code=400, content=BadRequestErrorSchema(detail=exc.detail).model_dump()
    )


async def conflict_exception_handler(
    request: Request, exc: exceptions.ConflictError
) -> JSONResponse:
    logger.warning(f'ConflictError: {exc.detail}')
    return JSONResponse(
        status_code=409, content=ConflictErrorSchema(detail=exc.detail).model_dump()
    )


async def internal_server_error_exception_handler(
    request: Request, exc: exceptions.InternalServerError
) -> JSONResponse:
...
def register_exception_handlers(app):
    ...
    app.add_exception_handler(
        exceptions.UnauthorizedError, unauthorized_exception_handler
    )
    ...
    app.add_exception_handler(
        exceptions.InternalServerError, internal_server_error_exception_handler
    )
